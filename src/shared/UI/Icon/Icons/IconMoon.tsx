const IconMoon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg
    width='800px'
    height='800px'
    viewBox='0 0 24 24'
    fill='none'
    xmlns='http://www.w3.org/2000/svg'
    {...props}
  >
    <path
      d='M20.4335 13.9335C20.1286 13.9773 19.817 14 19.5 14C15.9101 14 13 11.0899 13 7.5C13 7.18303 13.0227 6.87136 13.0665 6.56653C9.91893 7.0192 7.5 9.72711 7.5 13C7.5 16.5898 10.4101 19.5 14 19.5C17.2729 19.5 19.9808 17.0811 20.4335 13.9335Z'
      stroke='#000000'
    />
    <path d='M4.5 8.75V10.25' stroke='#000000' strokeLinecap='round' />
    <path d='M5.25 9.5H3.75' stroke='#000000' strokeLinecap='round' />
    <path d='M7.5 3.5V5.5' stroke='#000000' strokeLinecap='round' />
    <path d='M8.5 4.5H6.5' stroke='#000000' strokeLinecap='round' />
  </svg>
);

export default IconMoon;
